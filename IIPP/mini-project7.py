...
class Ship:
    ...
    def update(self):
        ...
        if self.thrust:
            ...
            self.vel[0] += acc * forward_vec[0]
            self.vel[1] += acc * forward_vec[1]
    ...
    # control thrust and sound    
    def control_thrust(self, thrusting):
    ...
    def shoot(self): 
        ...
        missile_vel = [0, 0]
        for i in range(2):
            missile_pos[i] = self.pos[i] + self.radius * forward_vec[i]
        for i in range(2):
            missile_vel[i] = self.vel[i] + 3 * forward_vec[i] 
...
def keyup(key):
    if key == simplegui.KEY_MAP["left"]:
        ...
        my_ship.control_thrust(False)    
        
def draw(canvas):
    global time, ship_angle
    ...
    #draw text and score
    canvas.draw_text("Lives: " + str(lives), [50, 50], 40, "Red")
    canvas.draw_text("Score: " + str(score), [600, 50], 40, "Red")     
        
# timer handler that spawns a rock    
def rock_spawner():
    global a_rock
    random_num = random.randrange(-10, 10, 1) / 5
    a_rock = Sprite([random.randrange(WIDTH), random.randrange(HEIGHT)],
                    [random_num, random_num], 1, random.random() / random.choice([5, -5]),
